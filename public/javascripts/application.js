...
var kuwest = function() {
    ...
    self.init = function() {
        ...
        $(".tweets").tweet({
            ...
            compose_tweet: function(avatar, date, join, text) {
                return text + '<div class="date">' + date + "</date>"
            }
        });
        kuwest.tip.init();
        kuwest.winform.init();
        ...
        $(".soon").click(function() {
            self.notice("Coming Soon.  But <a href=\"/users/new\">sign up now</a> and start leveling up!");
            return false;
        });
...
kuwest.comments = function() {
    ...
    self.init = function() {
        ...
        $("#expand_profile").toggle(
	        ...
	        function() {
	            $(".profile").fadeOut();
	            $(this).text("Show Profile Details...")
	        }).css("pointer", "cursor");
...
kuwest.winform = function() {
    ...
    self.init = function() {
        var text = $("#win").val();
        $("#status_form").inputHintOverlay(2, 25, {
            color: "#999"
        });
        $("form.quest").inputHintOverlay(5, 5, {color:"#aaa"});
        ...
        $("#status_form").submit(function() {
            if ($("#win").val() == text) {
                kuwest.error("Please enter some text.");
                return false;
            }
            kuwest.ajax.start();
            $.ajax({
...
$('.status_image img').live('click',
function() {
    var url = $(this).attr("src").replace(/small/, 'original');
    $.slimbox(url, '');
});
$('.profile_image img').live('click',
function() {
